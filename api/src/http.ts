...
export type Request = FastifyRequest<any> & {
  query: any;
  params: any;
  body: any;
...
async function runHandlers(handlers: Handler[], req: Request, res: Response, index = 0): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    const next = () => {
      advanced = true;
      void runHandlers(handlers, req, res, index + 1).then(resolve, reject);
    };
  ...
  for (const route of router.routes) {
    server.route({
      ...
      handler: async (request, reply) => {
        ...
        await runHandlers([...router.middleware, ...route.handlers], adaptRequest(request), response);
      },
    });
  }
}

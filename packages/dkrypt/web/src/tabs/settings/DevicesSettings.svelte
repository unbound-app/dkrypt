    ...
    <div class="grid gap-3 xl:grid-cols-2">
      {#each devices as device (device.id)}
        ...
        <div class="border-border/80 bg-background/30 min-w-0 rounded-xl border p-4">
          ...
          {#if h?.subsystems}<div class="mt-3 flex flex-wrap gap-1.5" aria-label="Device subsystem status">{#each Object.entries(h.subsystems) as [name, status]}<span class="rounded-full border px-2 py-0.5 text-[10px] {status === 'ready' ? 'border-ok/30 text-ok' : status === 'offline' ? 'border-err/30 text-err' : 'border-border text-muted'}">{name}: {status}</span>{/each}</div>{/if}

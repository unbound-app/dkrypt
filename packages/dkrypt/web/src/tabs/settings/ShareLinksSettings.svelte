<script lang="ts">
	...
	const canExtend = $derived(
		sessionHasPermission(PermissionFlag.extendShareLinks),
	);
	...
	async function revoke(linkId: string): Promise<void> {
		if (
			!(await confirmDialog(
				"Revoke this share link? It stops working immediately.",
			))
		)
			return;
	...
	$effect(() => {
		void ensureAppCatalog((allLinks ?? []).map((link) => link.bundleId));
	});
	...
	function linkStatus(
		...
		if (l.maxDownloads !== undefined && l.downloadCount >= l.maxDownloads)
			return "exhausted";
		return "active";
	}
	...
	function downloadsLabel(l: ShareLinkRecord): string {
		return l.maxDownloads !== undefined
	...
	const manageTtlOptions = $derived(
		manageLink ? ttlOptionsFor(manageLink) : [],
	);
...
<Card title="Share links">
	{#snippet headerExtra()}
		<div class="flex items-center gap-2">
			<a
				href={shareLinksExportUrl("csv")}
				download
				class={buttonVariants("secondary", "sm")}>Export CSV</a
			>
			<a
				href={shareLinksExportUrl("json")}
				download
	...
	<div class="mb-3 text-xs text-muted">
		Every active download share link issued across all jobs. Revoked links
		are hidden. Copy, manage, or revoke any of them.
	</div>
	...
	{:else if allLinks}
		<div class="flex flex-col gap-2">
			{#each links as l (l.id)}
				...
				<div
					class="border-border rounded-md border px-3 py-2.5 text-xs"
				>
					<div class="flex items-start justify-between gap-3">
						<div class="min-w-0 flex-1">
							<div class="flex flex-wrap items-center gap-1.5">
								<Badge variant={statusVariant(status)}
									>{status}</Badge
								>
								{#if appIconUrl(l.bundleId)}
									<img
										src={appIconUrl(l.bundleId)}
										alt=""
										class="h-4 w-4 shrink-0 rounded"
									/>
								{/if}
								<span class="truncate" title={l.bundleId}
									>{appDisplayName(l.bundleId)}</span
								>
							</div>
							<div
						...
						<div class="flex shrink-0 items-center gap-1.5">
							<Button
								size="sm"
								variant="secondary"
								onclick={() => openManage(l)}>Manage</Button
							>
							<Button
								size="sm"
								variant="destructive"
								loading={revoking.has(l.id)}
								onclick={() => revoke(l.id)}>Revoke</Button
							>
						</div>
					</div>
					...
					{#if l.url}
						<div class="mt-2 flex items-center gap-2">
							<code
								class="min-w-0 flex-1 truncate rounded bg-panel-muted px-1.5 py-1"
	...
	{#if manageLink}
		...
		<div class="flex gap-2">
			<div class="flex-1">
				...
				{#if manageTtlOptions.length > 0}
					<Select
						items={manageTtlOptions}
						bind:value={manageTtl}
						class="w-full"
					/>
				{:else}
					<div class="text-xs text-muted italic">
						no shorter option available
					</div>
				{/if}
			</div>
			<div class="flex-1">
				...
				<Select
					items={MAX_DOWNLOAD_OPTIONS}
					bind:value={manageMaxDownloads}
					class="w-full"
				/>
			</div>
		</div>
		{#if !canExtend}
			<div class="mt-2 text-xs text-muted italic">
				Only expiry times shorter than the current one are available
				without the extend permission.
			</div>
		{/if}
		<div class="mt-4 flex gap-2">
			<Button
				class="flex-1"
				loading={saving}
				disabled={manageTtlOptions.length === 0}
				onclick={saveManage}>Save</Button
			>
			<Button
				variant="secondary"
				class="flex-1"
				onclick={() => (manageOpen = false)}>Cancel</Button
			>
		</div>
	{/if}
</Dialog>

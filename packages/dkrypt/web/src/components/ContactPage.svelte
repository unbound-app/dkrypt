<script lang="ts">
  import Card from '#lib/components/ui/Card.svelte';
  import PublicPageFooter from '#components/PublicPageFooter.svelte';
  import PublicPageHeader from '#components/PublicPageHeader.svelte';
</script>

<div class="min-h-screen">
  <PublicPageHeader />
  <main class="mx-auto max-w-3xl px-5 py-12">
    <div class="mb-8">
      <div class="text-accent mb-3 text-sm font-semibold tracking-wide uppercase">Support</div>
      <h1 class="text-3xl font-semibold tracking-tight sm:text-4xl">Contact dkrypt</h1>
    </div>

    <div class="grid gap-4 sm:grid-cols-2">
      <Card>
        <h2 class="text-base font-semibold">Product and privacy support</h2>
        <p class="mt-2 text-sm leading-6 text-muted">
          For account, service, or privacy questions, email
          <a class="text-accent" href="mailto:contact@dylib.dev">contact@dylib.dev</a>.
        </p>
      </Card>
      <Card>
        <h2 class="text-base font-semibold">Billing and refunds</h2>
        <p class="mt-2 text-sm leading-6 text-muted">
          Open the manage billing link in dkrypt or email
          <a class="text-accent" href="mailto:contact@dylib.dev">contact@dylib.dev</a> with the purchase email and receipt details.
        </p>
      </Card>
      <Card>
        <h2 class="text-base font-semibold">Security reports</h2>
        <p class="mt-2 text-sm leading-6 text-muted">
          Do not post credentials or vulnerability details publicly. Send sensitive reports to
          <a class="text-accent" href="mailto:contact@dylib.dev">contact@dylib.dev</a>.
        </p>
      </Card>
    </div>
  </main>
  <PublicPageFooter />
</div>
